...
def init_thresholds_from_quantiles(
    ...
    This matches the notebook behavior: for each feature j, compute ~min(R, max_q)
    quantiles then tile to length R.
    """
    ...
    for j in range(D):
        qn = int(min(max(2, min(R, max_q)), max_q))
...
def sparsemax(logits: torch.Tensor, dim: int = -1) -> torch.Tensor:
    ...
    Notebook implementation kept as-is.
    """
...
def default_groups_for_D(D: int, G: int = 4) -> List[List[int]]:
    """
    Notebook default grouping heuristic: if D==8 use a fixed pattern, else split into G chunks.
    """
    if D == 8:
...
def build_feature_to_fact_mask(D: int, n_thresh_per_feat: int) -> torch.Tensor:
    """
    Assumes ThresholdFactBank orders facts per feature contiguously, count = D*n_thresh_per_feat.
    Then we augment facts with (1-f), so Fin = 2*F0. Returns [D, Fin].
    """
